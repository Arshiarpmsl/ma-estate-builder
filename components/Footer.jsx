...
export default function Footer({ company }) {
  return (
    <footer className="bg-slate-900 text-white py-16">
      <div className="max-w-7xl mx-auto px-4 grid md:grid-cols-4 gap-10">
        ...
        <div>
          ...
          {['Home','Services','Gallery','About','Reviews','Contact'].map(l =>
            <Link key={l} href={l==='Home'?'/':`/${l.toLowerCase()}`} className="block text-slate-400 hover:text-amber-500 mb-2">
              {l}
            </Link>
          )}
        </div>
        ...
        <div>
          ...
          <a href={`tel:${company?.phone}`} className="block text-slate-400 mb-2">
            <Phone className="w-4 h-4 inline mr-2"/>
            {company?.phone}
          </a>

          <a href={`mailto:${company?.email}`} className="block text-slate-400 mb-2">
      ...
      <div className="max-w-7xl mx-auto px-4 border-t border-slate-800 mt-12 pt-8 flex justify-between">
        <p className="text-slate-500">© 2026 {company?.name || 'MA Estate Builder Ltd'}</p>
      </div>
    </footer>
  )
}
